...
export const POST = withRequestId(
  async (req: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
    ...
    if (bet.status !== "CANCELLED") throw new ApiError("Conflict", "Bet is not in a refundable state", 409);
    ...
    const claim = await verifyClaim(parsed.data.txHash as `0x${string}`, "RefundClaimed", auth.addr);
    ...
    return ok({ bet: updated, refundUsdc: claim.amount.toString() });
  },
);
